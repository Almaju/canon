...
2. **Stdlib cleanup to current spec** — 🔶 in progress. `map.can`/`set.can` ported (`Value`/`Keys`/`Values`/`List` as anonymous constructors, `Inserted`/`Removed` as shape implementations); remaining: the json.can wrappers, http accessors, html helpers, the parser internals. `length` is ported (`Length = Int` merged across map.can/set.can); `contains` waits with the comparison predicates.
