...
- `Path = String` — a newtype; constructing it costs nothing but says
  what the string *is*.
- `Path.File()` — the **validated constructor** of `File`: it actually
  opens the file, so its signature is
  `(Path) -> Result<File, IoError>`. Opening can fail, so the type says
  so, and the `?` is forced at the call site — you cannot forget to
  handle a missing file.
- `File.read()` — `(File) -> Result<String, IoError>`. You can only
...
the program failing with the error value rather than printing — try
pointing the `Path` at a file that doesn't exist.
