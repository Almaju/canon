...
A bodied declaration must be named after the type it constructs -- and since that name is redundant, the declaration is written as an anonymous arrow (below). Anything else is a compile error, checkable from signatures alone. Declaring a body-less shape is itself a checker error until shapes can do something a result newtype cannot ([Functions § Shape or Result Newtype](./functions.md#shape-or-result-newtype)); there are no standing exceptions -- even the literal-interpolation hooks are ordinary result-newtype families (`Encoded = Json`, `Escaped = Html`).
...
| shape impl `ToJson = (Bool) => Json { ... }` | `Bool => Encoded { ... }` -- a family member of `Encoded = Json` |
...
| shape impl `ToJson = (Bool) => Json { ... }` | `Bool => Encoded { ... }` -- the family member of a result newtype |
