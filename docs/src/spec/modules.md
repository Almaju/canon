...
`canon add` does the copy. `canon add <ns>/<name>` vendors a package
that ships with the toolchain (`canon add` alone lists them) at the
toolchain's version; `canon add <git-url>@<tag>` clones a git tag and
vendors the repository's `src/` (`https://github.com/acme/greet@v1.2.0`
lands under `deps/acme/greet@1.2.0/`), so publishing a package is
pushing a tag. A copy at another version is replaced — the loader
accepts one version of a package — and removing a dependency is
...
The standard library is the bundled package named `canon` — the
namespace alone, no second segment — and ships inside the compiler
binary. It is pre-installed but
...
The packages beside it in the binary — `canon/<name>`, the ecosystem
built on the prelude — are not resolved by name: a program sees one
once `canon add` has vendored it under `deps/`. The prelude's names
are the only global ones.
