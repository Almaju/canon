...
- **`main` and synthesised entries** are exempt from the
  function-declaration rule — the entry point is distinguished by role.
...
canon fmt file.can          # rewrites to canonical form
```

`canon check` and `canon run` refuse files that aren't canonically
formatted, so ordering violations surface immediately rather than in
review.
