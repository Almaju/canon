...
canon doc packages              # every package under a directory, as one site
```
...
package's tests are its examples: every test in a `*_test.can` under
`src/` appears on the page of each type it names, captioned by its
name (`SegmentsKeepOrder` reads as "Segments keep order"), so a test
suite written as sentences is a worked reference for free. The
reference of the prelude and every package beside it, generated by
this command over `packages/`, is [published beside these
docs](api/index.html).
