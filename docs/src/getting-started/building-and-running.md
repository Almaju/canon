...
```sh
canon add https://github.com/acme/greet@v1.2.0
```

Clones the tag and vendors the package's `src/` under
`deps/acme/greet@1.2.0/`, where a reference to any type it declares
resolves ([Modules § Dependencies](../spec/modules.md#dependencies-deps)).
The directory is the version pin; delete it to drop the dependency. To
publish a package, push a tag.
