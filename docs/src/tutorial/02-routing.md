...
The service says the same thing on every path. Time to route. Canon has
...
`String` and `Int`** scrutinees. Arms are literals, and the final arm
is a mandatory catch-all naming the scrutinee's type; literals can
never be exhaustive, so totality comes from the catch-all. Update
`notes.can`:
...
the `(String)` catch-all is the 404, "any other path". Inside every arm
the path value stays in scope as `String`, so the catch-all could echo
it back.
...
There is no `if`/`else` to compare paths with: an `if` is a dispatch on
`Bool` with better marketing. Canon keeps the one construct. Unions
dispatch by variant; `String`/`Int` dispatch by equality with a
catch-all. See [Dispatch](../tour/match.md) in the Tour for the full
argument.
