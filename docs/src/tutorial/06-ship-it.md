...
Everything so far ran under `canon run`. The last step is realizing that
`canon run` was never special: the thing it runs is a standard artifact
...
- **`notes-api.wasm`** — a WebAssembly **Component** (WASI Preview 3).
  Not a Canon-specific binary: a self-describing component that imports
  only standard `wasi:*` interfaces and exports
...
- **`notes-api.wit`** — a human-readable description of the component's
  *world*: everything it imports and exports. Feed it to `wasm-tools`
  or any Component Model tooling; this file is the component's contract.
