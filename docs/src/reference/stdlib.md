...
no files to install and nothing to import. Reference a type by name —
`Random()`, `Instant()`, `Url("…")` — and the compiler loads its module
automatically (see [Name Resolution](../spec/modules.md)).
...
Idiomatic code only ever sees the curated layer. The `wasi/…` binding
files never participate in name resolution — they are reached only
through explicit alias declarations
(`now = wasi/clocks/monotonic_clock/now`) — and are regenerated by
`just regen-bindings` against the vendored WIT under `wit-vendor/wasi/`.

## At a Glance

| Module (std) | Type | Backing binding | Notes |
|---|---|---|---|
...
| `fs/Path` | `Path = String` | — | filesystem path newtype |
...
| `http/HttpError` | `HttpError = String` | — | HTTP-client error newtype |
| `http/HttpServer` | `HttpServer = String` | `canon:builtins/http-server` | HTTP/1.1 server, see [HTTP Server](#http-server) |
| `http/Port`, `http/RoutePath`, `http/HttpStatus`, `http/Body` | various | — | HTTP-server helpers |
...
need it directly — most users should reach for `canon/std/time/Now` instead.
