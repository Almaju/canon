...
Every import is a standard `wasi:*` interface — cli, clocks, random,
files, the HTTP client and the HTTP handler surface — so a component
runs on any host that provides them. File access goes through
