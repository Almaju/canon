...
impl<'m> WasmGen<'m> {
    ...
    /// (n) bytes. All three locals are consumed deterministically: the
    /// loop exits with `rptr = dst + n`, `rbool = src + n`, and
    /// `rlen = 0` — `compile_stream_to_string` relies on `rptr` ending
    /// at the next write position to chain copies.
    ///
    ...
    fn list_loop_trio_ty(&self) -> u32 {
        self.user_type_map
            ...
            .copied()
            // invariant: every encoder mode reserves this loop type in
            // `user_type_map` before any function body is compiled.
    ...
    /// string-shaped slots as `(ptr, len)` into `map_elem_ptr`/`+1`.
    fn bind_list_elem(&self, elem_repr: &Ty, scope: &LocalScope, f: &mut Function) {
        match elem_repr {
            Ty::I64 => {
    ...
    fn inline_unary_lambda(&self, args: &[Expr]) -> Option<(String, Ty, Block)> {
    ...
    /// Compiles `stream.filter(lambda)` as a fused loop over the
    /// list-backed stream. Structure mirrors `compile_list_map`: the
    /// loop-carried state `(src, dst, remaining)` lives on the operand
    /// stack (the reserved i32-trio block type) while the predicate body
    /// runs, so arbitrary user code inside the lambda can't clobber it.
    /// Kept elements are copied as raw 8-byte slots — the slot layout is
    /// identical for every element repr, so one `i64` move covers both
    /// scalar and string-shaped elements. The destination is allocated
    /// at full source length (a safe upper bound); the final length is
    ...
    /// Compiles `stream.toString()` — the `Stream<String>` sink — as a
    /// two-pass join of the list-backed stream's elements into one
    /// freshly-allocated string: pass 1 sums the element lengths, pass 2
    /// copies each element's bytes consecutively. No user code runs
    /// inside either loop, so the scratch locals are used freely.
    pub(super) fn compile_stream_to_string(&mut self, scope: &LocalScope, f: &mut Function) -> Ty {
        let mem32 = MemArg {
            offset: 0,
            align: 2,
            memory_index: 0,
        ...
        // Stack: [ptr, len].
        f.instruction(&Instruction::LocalSet(scope.map_elem_ptr() + 1)); // n
        f.instruction(&Instruction::LocalSet(scope.map_elem_ptr())); // base

        // ── Pass 1: total = Σ element lengths ───────────────────────
        f.instruction(&Instruction::I32Const(0));
        f.instruction(&Instruction::LocalSet(scope.tmp_i32_b())); // total
        f.instruction(&Instruction::LocalGet(scope.map_elem_ptr()));
        f.instruction(&Instruction::LocalSet(scope.addr_scratch())); // cursor
        f.instruction(&Instruction::LocalGet(scope.map_elem_ptr() + 1));
        f.instruction(&Instruction::LocalSet(scope.tmp_i32())); // rem
        f.instruction(&Instruction::Block(BlockType::Empty));
        f.instruction(&Instruction::Loop(BlockType::Empty));
        ...
        // out = alloc(total + 1) — the +1 keeps an all-empty join from
        // handing $alloc a zero size.
        f.instruction(&Instruction::LocalGet(scope.tmp_i32_b()));
        f.instruction(&Instruction::I32Const(1));
        f.instruction(&Instruction::I32Add);
        f.instruction(&Instruction::Call(self.fn_alloc));
        ...
        // ── Pass 2: copy each element into place ────────────────────
        // `emit_byte_copy_loop` exits with `rptr` at the next write
        // position, so the write cursor self-maintains across
        // iterations — only src (rbool) and n (rlen) are set per
        // element.
        f.instruction(&Instruction::LocalGet(scope.alloc_ptr()));
        f.instruction(&Instruction::LocalSet(scope.rptr()));
        f.instruction(&Instruction::LocalGet(scope.map_elem_ptr()));
    ...
    pub(super) fn compile_builtin_method(
        ...
        match (method, &recv_ty) {
            ...
            // ── Stream combinators (canon:builtins/stream) ───────────────
            // Streams are eager, list-backed values at codegen: a
            // `Stream<T>` is the same contiguous (ptr, len) pair as
            // `List<T>`, so the source/sink combinators are identities
            // and the transformers are list loops. `map` and `concat`
            // need no arms of their own — `Mapped` / `Joined` on a
            // stream land on the `("map", Ty::List)` /
            // `("concat", Ty::List)` builtins directly.
            ("streamOf", Ty::List) | ("toList", Ty::List) => Ty::List,
            ("take", Ty::List) => {
                // `take(n)` clamps the length — list slots are
                // contiguous, so the prefix IS the taken stream.
                // Negative n clamps to 0; n past the end keeps
                // everything. Stack: [ptr, len].
                self.compile_i64_arg(args, scope, f);
                f.instruction(&Instruction::I32WrapI64);
                f.instruction(&Instruction::LocalSet(scope.tmp_i32())); // n
                f.instruction(&Instruction::LocalSet(scope.tmp_i32_b())); // len
            ...
            ("toString", Ty::List) => self.compile_stream_to_string(scope, f),
            ("get", Ty::List) => {
