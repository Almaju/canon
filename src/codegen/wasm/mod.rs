...
// (See `ty.rs` for `Ty`, `IndirectReturnShape`, and `ret_area_size_for`.)
...
// (`IndirectReturnShape`, `RecordField`, and `ret_area_size_for` live in `ty.rs`.)
...
impl<'m> WasmGen<'m> {
    ...
    fn emit_async_call(&mut self, info: &FuncInfo, scope: &LocalScope, f: &mut Function) -> Ty {
        ...
        if has_result {
            ...
            let size = ty::ret_area_size_for(&info.result_ty);
