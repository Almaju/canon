...
/// `extern Wasm.async` ones with `CanonicalOption::Async`, per
/// `ExternImport::is_async`.
pub(super) fn wrap(core_module: &[u8], externs: &[ExternImport]) -> Vec<u8> {
...
/// `wit-bindgen`.
///
/// `async_set` is surfaced as the async-inference summary comment — the
/// list of functions the binary lowers with `CanonicalOption::Async`.
pub(super) fn generate_wit(module: &OModule, async_set: &AsyncSet) -> String {
    ...
    // verify the bottom-up fixpoint matches their expectations.
