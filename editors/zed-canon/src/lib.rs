...
struct CanonExtension {
    cached_binary_path: Option<String>,
}

impl zed::Extension for CanonExtension {
    fn new() -> Self {
        Self {
    ...
    fn language_server_command(
        ...
        // 2. Check if `canon` is on PATH
        if let Some(path) = worktree.which("canon") {
        ...
        if let Ok(release) = release {
            ...
            // Look for a matching asset: canon-{arch}-{os} or similar
            let asset_name = format!("canon-{}-{}", arch, os);
            let asset = release.assets.iter().find(|a| {
                (a.name.starts_with("canon-") || a.name == "canon")
                    && (a.name.contains(&asset_name)
                        || a.name.contains(arch) && a.name.contains(os))
            });

            if let Some(asset) = asset {
                let binary_path = format!("canon-{}", release.version);
                ...
                zed::download_file(&asset.download_url, &binary_path, file_type)
                    .map_err(|e| format!("failed to download canon: {e}"))?;
                ...
                {
                    format!("{}/canon", binary_path)
                } else {
                    binary_path.clone()
                };

                zed::make_file_executable(&bin_path)
                    .map_err(|e| format!("failed to make canon executable: {e}"))?;
        ...
        // 4. Fallback
        Err("canon not found. Install it with: cargo install --path . (or add it to PATH)".into())
    }
}

zed::register_extension!(CanonExtension);
