...
|---|---|
| **Conversion-is-construction batch: `String(Int)` / `String(Byte)` / `Int(String)` / `String` ordering / `Float` comparisons / `list.Json()`** | The self-hosting prerequisite pass (DESIGN.md § Conversions is the language-level rule it introduced). (1) `String(Int)` / `Int.String()` — decimal rendering via the new `fn_int_to_str` helper (`build_int_to_str`, digit loop shared with `print_int`'s approach but copying out of the shared buffer into `$alloc` so chained renders can't clobber); replaces the web package's `toText` (deleted). (2) `Byte = Int` newtype (`canon/std/Byte`) + `String(Byte(65))` = `"A"` — byte-ness is detected *statically* (`expr_is_byte`: `Byte(…)` ctor, ident whose alias chain hits `Byte`, or `.Byte` field); a method chain returning `Byte` erases to `Int`, so wrap it. (3) `Int(String) -> Result<Int, MalformedInt>` — pure-Canon parser in `canon/std/Int` following the `Json` validator pattern (acc as bare-`Int` receiver, `Pos = Int` newtype for the cursor — receivers must be the underlying prim for dispatch, see the scalar-newtype gotcha); `compile_constructor` routes `Int(str-arg)` to the `(Some("String"), "Int")` func-table entry, and the checker types primitive ctors by arg: matching arg = identity, non-matching = the self-named method's declared return. (4) `String.lt/le/gt/ge/ne` via the new `fn_str_cmp` helper (byte-wise -1/0/1) — mirrors `Int`'s comparison surface. (5) `Float` comparisons had NO codegen (checker-accepts-drops-everything); added `F64Lt/Le/Gt/Ge/Eq/Ne` arms + `rem` as `a - trunc(a/b)*b` (wasm has no f64 rem; needed the second f64 scratch `tmp_f64_b`). Checker's `lte`/`gte` aliases (accepted, never compiled) deleted — `le`/`ge` is the one way. (6) `Map`/`Set` checker stubs removed — they typechecked with zero codegen behind them; honest "no method" errors until real codegen lands (`tests/checker/fail/map_set_not_implemented.can`). Two new defined-function slots shift `fn_user_start` in all three encoder modes (CLI/HTTP/web). Pinned by `tests/runtime/{float_compare,string_compare,int_to_string,byte_to_string,int_parse,int_parse_try}.can`. |
| **JSON prelude — literals need no `use canon/std/Json`** | Two halves. (1) Checker: `collect_symbols` registers `Json` as a known type aliased to `String` whenever the module doesn't define one, so a static literal (`{"k":"v"}` — a compile-time constant) gets `String`'s methods and `-> Json` annotations import-free, in every world including `wasi:http/service`. (2) Loader: `inject_json_prelude` in `src/loader.rs` walks each file's exprs and auto-processes a synthetic `use canon/std/Json` when the file actually reaches for the machinery — a literal with an *interpolated* part, a `Json(...)` constructor, or a `.ToJson()`/`.Json()` call — unless the file imports/defines `Json` itself. Side effect: interpolation in an HTTP handler now fails at build with the clear "world can't satisfy canon:builtins/json" error instead of silently compiling a dropped `ToJson` (the old no-import path produced an empty response body). Pinned by `tests/runtime/json_prelude_{static,interp,validator}.can` and `tests/checker/ok/http_json_literal_static.can`. |
| **`parallel`/`race` are methods, not bare calls** | `a.parallel(b)` / `a.race(b)` — the ordinary commutative method-call shape; the bare `parallel(a, b)` form is a checker error steering to the method spelling (Canon has no bare free-function call form anywhere else). Plumbing: checker accepts the combinators as methods with arity 1 and types them in `expr_type_name_in_scope`'s MethodCall arm (`parallel` → `List`, `race` → receiver's type); the auto-await pass exempts these two method names from receiver-awaiting (both sides must stay un-awaited futures); codegen intercepts at the top of `compile_method_call` — before the receiver is compiled — and routes `[receiver, arg]` into the existing `compile_parallel`/`compile_race` waitable-set sequences. Pinned by `tests/runtime/{parallel,race}_two_echoes.can` (method form) and `tests/checker/fail/concurrent_bare_call.can`. |
...
| `List<String>.Json()` builtin (né `toJsonArray`) | New helper function `fn_list_to_json_array` (`build_list_to_json_array` in `src/codegen/wasm/mod.rs`) slotted alongside `fn_alloc`, `fn_print_*`. Core signature `(list_ptr: i32, list_len: i32) -> (i32, i32)`. Two-pass algorithm: pass 1 sums `2 + sum(elem_len) + max(0, len-1)` to size the output, pass 2 fills it with `[`, comma-separated elements, `]`. The checker recognises `Json` as a List method returning `Json` (`is_known_method` + `method_return_type` in `src/checker/mod.rs`) — renamed from `toJsonArray` when the conversion-is-construction convention landed (DESIGN.md § Conversions). Pinned by `tests/runtime/list_to_json_array.can`. Pairs naturally with the `FromJson` primitives already in place — `someList.map(toJson).Json()` is the canonical pattern (once `list.map(f)` does more than identity, see the `List<T>` iteration gap). |
