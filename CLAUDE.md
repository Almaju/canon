...
| **WASI HTTP handler: request introspection & response composition** | Slice 1b landed: a `(Request) -> Response` program compiles to a standard `wasi:http/service` component and serves under `canon run --addr` (see the closed-gap row below). But the emitted handler ignores the user function's body — every response is an empty-bodied 200. The user can't read the request (method, path, headers, body) or set the status/headers/body. | Slices 2–3 of `WASI-HTTP-HANDLER.md`. The codegen work is to compile the user's handler body into the self-contained core module `build_http_service_core_module` emits (today the body is fixed), importing `wasi:http/types` accessors (`[method]request.get-path-with-query`, `[method]response.set-status-code`, …) via the same `wit-component` name-mangling conventions the slice-1b sequence uses. The stdlib `Response`/`Headers` bindings in `packages/canon/std/src/http/` also need re-pointing: they bind `[constructor]response`, which doesn't exist in the real rc WIT (`[static]response.new` with a trailers future is the actual constructor). |
