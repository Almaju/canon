...
| `src/codegen/` | WebAssembly component gen (`wasm/compile.rs` lowers expressions and method calls — constructors in `construct.rs`, dispatch in `dispatch.rs`, the builtin vocabulary in `builtins.rs`, list lambdas in `lists.rs`, I/O and concurrency in `effects.rs` — `wasm/mod.rs` assembles the self-contained core module, `wasm/stream.rs` is the `Stream<String>` runtime (stage structs pulled through a funcref table), `wasm/component.rs` embeds the program's world and lets `wit-component` emit the component) |
...
| `src/add.rs` | `canon add` — vendors a package to `deps/<ns>/<name>@<version>/`, replacing another version: a bundled `<ns>/<name>` at the toolchain's version, or `<git-url>@<tag>` cloned at the tag. |
| `src/doc.rs` | `canon doc` — the generated API reference. Parses a package's `src/` raw (not through `resolve_new_syntax`, which rewrites declarations into their codegen shape) and renders a static site: a page per type with its definition, its constructors, and every constructor accepting it; a page per module with its declarations. A directory of packages renders as one site with an index (`canon doc packages`, deployed at `/doc/api/`). |
...
| `build.rs` | Walks `packages/` → bundled-package registry baked into the binary. A dir with `src/` is a package named by its path (`packages/canon/` → `canon`, `packages/canon/ansi/` → `canon/ansi`). |
| `packages/canon/` | The standard library — the **prelude**, the one package resolved by name without vendoring. Hand-written wrappers under `src/`, WIT-derived bindings under `bindgen/` (committed), vendored upstream WIT under `wit/` (the import declaration — no manifest). |
| `packages/canon/<name>/` | The ecosystem packages (`canon/ansi`, …): bundled in the binary but invisible to name resolution until `canon add canon/<name>` vendors one into a project's `deps/`. Pure Canon over the prelude, independent of each other, each with its own `src/`. In-repo consumers (tests, examples, the docs app) vendor them as `deps/canon/<name>@<version>` symlinks into the package's `src/`. |
...
| `canonc/` | The self-hosted compiler, written in Canon. `src/main.can` is the entry and the only host boundary — it reads the path named on the command line and prints what `src/compiled.can` gives back. `src/compiled.can` is the compiler proper, one self-contained file with no imports, which is what `canonc` reads when it reads itself (`tests/canonc_self_hosts.rs`). It tokenizes the source into a cons-list (`Name` / `Number` / `Punct` / `Quoted`, the scanner carrying a mode so a backtick literal's text is text until a `{` and a hole's contents are scanned by the ordinary rules, with `=>` its own punctuation so a declaration head can be told from the type declarations that precede it), parses a brace body as an operand followed by a chain of steps — `-> Op(operand)` for the arithmetic and comparison operators, `-> Name` or `-> Name(operand)` for a call to another declaration (resolved through a table of every declared name built before any body is parsed, so calls may point forward, and recursion and mutual recursion both compile), or `-> ( * False { … } * True { … } )` for a dispatch, which is wasm's `if` — where an operand is a literal, a parameter, `Name()` for a `Unit` declaration or a payload-less variant, a prefix construction `Name(operand)`, or a parenthesised body, and an argument list takes as many components as the callee has, separated by `*`; a type is read through its aliases to the declaration that gives it a shape — or a string literal, which lowers to the reference compiler's `(ptr, len)` pair into a data segment, so the module carries a memory it exports — the value in hand carries the name of its type through the chain, so the operations in reach are the ones that type has (`ByteAt`, `Eq`, `Joined`, `Length` and `Substring` on a string, a pipe into a declared type is a call when the name has a body that takes the value's type (a number literal takes any `Int`- or `Bool`-rooted input, a string literal any `String`-rooted one) and a relabel otherwise, whichever side of the chain the value came from, and a prefix `Name(x)` in operand position follows the same rule, a product is a pointer to its fields laid out in declaration order (four bytes a scalar, eight a string), `.` reads one back and a pipe into the type builds one, each value landing in the field whose type it carries; a union is a pointer to a tag and a payload as wide as its widest variant needs, a pipe into it tags the value — unless the value already roots at that union, which is a relabel — and a dispatch on one tests that tag arm by arm and binds the payload under the arm's name; a dispatch on a literal tests each arm — the equality helper for a string, `i32.eq` for a number — and takes the catch-all last; a body is expressions, and every one but the last is dropped, and a backtick literal folds into a `Joined` chain over its text chunks and holes; every allocation goes through a helper that bumps a global starting past the data segment and grows memory to fit — nothing is freed, matching the spec's arena — and `Eq`, `Int -> String` and that allocator are prepended at indices 0, 1 and 2 so a call's target is knowable before the file's declarations are counted), and a declaration returning one has two results — since one parse step reads every body and takes its terminator (`}` or `)`) as an input, and emits a WebAssembly core module exporting each declared type, lowercased, as a function evaluating it — taking one `i32` per name the head puts before `=>`, or two for a name whose type declaration roots at `String`; anything the grammar doesn't accept is a diagnostic naming the token it wanted, prefixed with the declaration it came from. A call's arguments are routed by type, not position: each is parked in a scratch local at the offset its type names among the callee's inputs and read back in declaration order, so the piped receiver lands wherever its own type sits in the head. Scratch is a window per nesting depth — the dispatch scrutinee, the product cell being filled, the parked arguments — and an argument list pushes a depth so a call inside one cannot tread on the call it sits in. Emits hex, since Canon cannot write binary yet; `tests/canonc_emits_wasm.rs` decodes it, runs it on wasmtime and checks the result. **It self-hosts** (`tests/canonc_self_hosts.rs`): the module `canonc` emits for `src/compiled.can`, handed that same source, emits the same bytes. The Rust compiler is the bootstrap host, no longer a dependency. Not yet in its subset: constructor families (it exports one body per type) and `T^N` heads. |
...
- **No package manifest, no reserved filenames** — file structure is the whole
  ...
  **workspace** is a dir whose immediate subdirs include packages. **External
  imports** = the `wit/` dir; **dependencies** = `deps/<ns>/<name>@<ver>/`, vendored by `canon add` — a bundled `<ns>/<name>` at the toolchain's version, or `<git-url>@<tag>` (the tag is the version); the directory is the lockfile. The
