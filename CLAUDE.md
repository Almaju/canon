...
| `src/bindgen/` | `oneway bindgen` — WIT → Oneway source emitter (`naming.rs`, `emit.rs`, `mod.rs`) |
...
| `packages/oneway/std/` | The standard library — one shipped package. Contains hand-written wrappers under `src/`, WIT-derived bindings under `bindgen/` (committed), and an `oneway.toml` declaring its WIT imports under `[imports]`. The loader resolves `use oneway/std/X` against this tree. |
| `packages/oneway/std/bindgen/` | Generated WASI bindings (produced by `just regen-bindings`, which runs `oneway install packages/oneway/std`). Committed so `cargo build` works on a fresh clone; treated as a derived artifact — never hand-edited. |
| `wit-vendor/wasi/` | Vendored upstream WIT files — source for `packages/oneway/std/bindgen/`. Bumped when WASI advances. |
...
| **WIT `resource` / `own<T>` / `borrow<T>` in `extern Wasm` signatures** | The canonical-ABI requires a `(type $Foo (resource (rep i32)))` declaration and `(own $Foo)` / `(borrow $Foo)` parameter/return shapes; the current extern-import lowering only knows scalar `PrimitiveValType`s and strings. Even unused imports fail validation. | `oneway bindgen` emits the resource *types* as `Foo = Handle` newtypes (see DESIGN.md §Resources), but skips every resource method / constructor / static, and skips any free function whose signature transitively mentions a handle, with reason `"... (codegen lowering pending, see CLAUDE.md)"`. This drops the entire `wasi:filesystem/types#descriptor` method surface and the `wasi:sockets/*` resources. Fix in `src/codegen/wasm/mod.rs` extern-import lowering: declare resource types in the component type section, extend `ParamKind` with `OwnedResource` / `BorrowedResource` variants, and emit ownership-tagged params/returns. Linear-use checking belongs in a separate checker pass once lowering works (slice 4 in the resources plan). |
| **`build_product_value` clobbers `scope.alloc_ptr` on nested constructors** | Building `Message(Some("hi"), "alex")` corrupts the outer struct because `build_option_some` overwrites the same `alloc_ptr` local. Symptom: field reads return the wrong value (e.g. dispatch picks the `None` arm even though `Some` was constructed). | `build_union_value` solved this with a pre-compile-to-scratch-locals pass (`SavedField::Ptr0`/`Ptr1`/`I64_0`/`Str0`); the product path needs the same treatment. Fix in `src/codegen/wasm/mod.rs` — mirror the `build_union_value` Step 1–3 structure inside `build_product_value`. Until then, products whose fields require nested allocation (any `Some(…)`, `Ok(…)`, nested product, etc.) silently produce wrong runtime values. |
| **`Foo.ToJson()` on a `NamedStr` newtype receiver** | `"x".ToJson()` produces `"\"x\""` (host-encoded JSON string), but `Foo("x").ToJson()` where `Foo = String` silently drops the call — nothing reaches stdout, and embedding it in a `{…}` JSON literal yields an empty interpolation (`{"k":,}`). | Method dispatch for the user-defined extern `ToJson = (String) -> Json` doesn't follow the newtype alias chain from `NamedStr("Foo")` back to `String`. Other receivers see the chain (e.g. `.print()` and `.concat()` on a `NamedStr` work fine), so the fix is localised to wherever extern-function method-call lookup keys the receiver type. Workaround: unwrap the newtype first (e.g. `Foo.String.ToJson()`) or pass the underlying string literal directly. |
| **Generic `list.get(i)` and `list.map(f)` for non-`Int` lists** | `build_list_literal` stores `Ty::Str`/`NamedStr` elements correctly (i32 ptr at slot+0, i32 len at slot+4); `List<String>.toJsonArray()` produces `[a,b,c]` via a synthesised helper function. But `list.get(i)` is still missing, and `list.map(f)` is still the identity for any element type. So you can construct a `List<String>` and convert it to a JSON array, but you can't pull individual elements back out from Oneway code. | Future work for proper iteration. The `toJsonArray` helper proves the per-element-width storage works end-to-end; `list.get(i)` is the natural next builtin (branch on the expected element type, read back the right number of bytes from `list_ptr + i*stride`). |
| **WASI HTTP handler export** | `oneway:builtins/http-server` is a private host bridge — components depending on it can only run inside `oneway run`. The target is the standard `wasi:http/handler.handle` export, runnable by any compliant host (`wasmtime serve`, Spin, etc.). | Architecture is captured in `WASI-HTTP-HANDLER.md` at the repo root, which supersedes the earlier `DYNAMIC-HANDLERS.md`. Slice 0 (stdlib types + design doc) and slice 1a (parser + checker entry-detection rule) have landed; slice 1b is the codegen work — `wasi:http/service` world emission and minimal resource lowering for `request`/`response`/`headers`. Slices 2–4 then layer in request introspection, response composition, and `oneway run --addr` end-to-end. Resource lowering is the longest pole and is shared with the other resource-heavy gaps (filesystem, sockets, HTTP client). |
| **`Stream<T>` lowering + streaming response bodies (SSE / NDJSON)** | The current SSE pathway is a one-shot `Content-Type:` prefix on the handler's return `String` (see the closed-gap row below) — no per-event streaming. Real streaming needs `Stream<T>` as a canonical-ABI handle, a `oneway/std/Stream` combinator surface (`map` / `filter` / `take` / `concat`), and a `(Headers * Status * Stream<String>) -> Response` constructor that pipes the stream into `wasi:http/types`' `writable-body`. Today the codegen silently drops imports whose signatures mention `Stream<T>` (`build_extern_component_params` returns `None` on unknown-shape params), so a program that imports stream combinators fails at runtime with "matching implementation was not found in the linker" — the imports were never even emitted. | Architecture in `STREAMING.md`. Slice 0 (stdlib skeleton + `tests/checker/ok/stream_compose.ow` + the `Stream<T>` no-peel fix in `method_return_summary`) and slice 1a (`tests/wit_component_stream_prototype.rs` proving `wit-component` lifts `stream<u8>` params/returns without per-stream codegen) have landed. Slice 1b is the integration push: route programs whose surface mentions `Stream<T>` (or any `Handle`-typed param) through `ComponentEncoder::module(…)` instead of the hand-rolled `wasm-encoder` type section — the same architectural shortcut from the WASI-HTTP-HANDLER row. Slices 2–4 layer combinators (pure Oneway), the `Response` streaming-body constructor, and a real time-driven endpoint test on top. SSE itself is two helpers in `oneway/std/http/sse.ow` (`eventStream`, `formatSse`) — no primitive, no host bridge. |
...
| String primitives (`length`, `byteAt`, `substring`, `eq`) | New built-ins in `compile_builtin_method` in `src/codegen/wasm/mod.rs`. Enable pure-Oneway parsers / validators over strings (see `packages/oneway/std/src/json.ow`). |
...
- Standard library is **layered** but ships as a single bundled package, `oneway/std`. The package's manifest declares its WIT dependencies under `[imports]`; `oneway install` materializes the bindings into `packages/oneway/std/bindgen/<ns>/<pkg>/<iface>.ow` (one file per interface; each function declared with a bare `extern Wasm` marker). The hand-written wrappers under `packages/oneway/std/src/` then `use wasi/…` to consume them, exactly as user code would `use wasi/…` against bindings installed into its own `bindgen/` directory. There is no privileged shape that only the stdlib can use — the bundled-package and project-`bindgen/` lookups are different code paths in the loader but cover the same `use` paths from the source's point of view. Users only ever `use oneway/std/…`. The compiler's runtime fulfils the WASI imports through `wasmtime_wasi::p3`.
- The `packages/oneway/std/bindgen/` tree is regenerated by `just regen-bindings` (which is just `oneway install packages/oneway/std`). Don't hand-edit it; bump the vendored WIT and regenerate.
- A bindings file declares its provenance with a single `bindings "<ns>:<pkg>/<iface>@<ver>"` directive at the top, then lists the bound functions as bare function-type aliases (`name = (P) -> R`). The loader (see `apply_bindings_directive` in `src/loader.rs`) rewrites those aliases into FunctionDefs with `extern_wasm.path` set to `<urn>#<fn-kebab>`. This is what `oneway install` emits and what every file under `bindgen/` looks like.
...
- Each `bindgen/` directory also contains an `_install.toml` sidecar written by `oneway install`: a map from `<rel-path>.ow` to the WIT interface URN that file was generated from. It's a derived artifact (committed for `oneway/std` so `cargo build` works on a fresh clone, gitignored for user projects). The runtime parser lives in `src/install.rs::parse_install_index`; `build.rs` has its own minimal parser because it can't depend on the crate it's building.
- Manifest schema: `[deps]` declares Oneway-package dependencies (`"name" = "version"`), `[imports]` declares external bindings (`"<path-prefix>" = "<source>"` where source is a local `.wit` file, a directory of `.wit` files, or a `.wasm` component; remote sources are deferred). Both tables are alphabetical. See `src/manifest.rs` for the parser and `src/install.rs` for the install logic.
