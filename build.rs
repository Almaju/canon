...
//!
//! Today the only shipped package is `oneway/std`, which contains both the
...
//! prior `oneway install` step). Both directories flatten into the same
...
fn discover_packages(root: &Path) -> Vec<DiscoveredPackage> {
    ...
    for namespace_entry in fs::read_dir(root).expect("read packages dir").flatten() {
            ...
            .flatten()
        {
            ...
            // Bindings generated by `oneway install` live under
            // `<package>/bindgen/`. They share the same flat namespace as
            // `src/` from the bundle's perspective — a file at
            // `<package>/bindgen/wasi/random/random.ow` is bundled with
            // `rel_path = "wasi/random/random.ow"`, the same form a
            // hand-written file at `<package>/src/wasi/random/random.ow`
            // would have. This is what lets `oneway/std`'s own source
